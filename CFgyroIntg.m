...
function [gyro_x,RMSE_CF] = CFgyroIntg(Acc,Gyro,Mag,Quat,fs)
 for i = 1:length(Acc)
...
%% Calculate Phi, Theta, and Psi using  and Gyroscope Measurements  
    phi_gyro    =   zeros(length(Acc),1);
    theta_gyro  =   zeros(length(Acc),1);
    psi_gyro    =   zeros(length(Acc),1);
    phi_gyro(1,1)   =   phi2_acc(1,1);
    theta_gyro(1,1) =   theta1_acc(1,1);
    psi_gyro(1,1)   =   psi_mag(1,1);
